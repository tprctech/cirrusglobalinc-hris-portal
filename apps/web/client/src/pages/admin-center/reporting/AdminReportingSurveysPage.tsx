...
type AdminReportingSurveysPageProps = {
  onNavigate?: (path: string) => void;
};

const API = '/api/v1';

function AdminReportingSurveysPage({ onNavigate }: AdminReportingSurveysPageProps) {
  ...
  useEffect(() => {
    fetch(`${API}/survey-campaigns`)
      .then((r) => {
        if (!r.ok) throw new Error('Failed to load survey campaigns');
        return r.json();
      })
      .then((data) => setCampaigns(data))
      .catch((err) => setError(err.message || 'Failed to load survey campaigns'))
      .finally(() => setLoading(false));
  }, []);

  const loadCampaignDetail = useCallback((campaignId: number) => {
    ...
    Promise.all([
      fetch(`${API}/survey-campaigns/${campaignId}`).then((r) => {
        if (!r.ok) throw new Error('Failed to load campaign details');
        return r.json();
      }),
      fetch(`${API}/survey-campaigns/${campaignId}/responses`).then((r) => {
        if (!r.ok) throw new Error('Failed to load responses');
        return r.json();
      }),
    ])
      .then(([detail, resps]: [CampaignDetail, Response[]]) => {
    ...
    return (
      <div className="admin-center-page">
        <div className="admin-center-shell">
          <AdminCenterSidebar activeMenu="reportingSurveys" onNavigate={onNavigate || (() => {})} />
          <div className="admin-center-content">
            <div className="admin-users-section">
              <div className="admin-users-toolbar">
                <div>
                  <h1 className="admin-breadcrumb-title">
                    ...
                    {campaignDetail?.title || 'Survey Campaign'}
                  </h1>
                  <p>
                    {submittedResponses.length} submitted response{submittedResponses.length !== 1 ? 's' : ''}
                    {campaignDetail && ` | Scope: ${campaignDetail.scope}`}
                    {campaignDetail?.due_date && ` | Due: ${campaignDetail.due_date}`}
                  </p>
                </div>
              </div>
              ...
              {!detailLoading && !error && campaignDetail && (
                <>
                  ...
                  {viewMode === 'summary' && (
                    <div className="reporting-sections">
                      ...
                      {campaignDetail.sections.length === 0 && (
                        <p className="reporting-no-data">No sections found for this survey campaign.</p>
                      )}
                    </div>
                  )}
          ...
          <div className="admin-users-section">
            <div className="admin-users-toolbar">
              <div>
                ...
                <p>Select a survey campaign to view collected responses.</p>
              </div>
            </div>

            {error && <p className="reporting-error">{error}</p>}
            {loading && <p className="reporting-loading">Loading survey campaigns...</p>}

            {!loading && !error && campaigns.length === 0 && (
              <div className="reporting-empty-state">
                <Users size={40} />
                <p>No survey campaigns found. Create survey campaigns from the employee portal to start collecting responses.</p>
              </div>
            )}
            ...
            {!loading && !error && campaigns.length > 0 && (
              <div className="reporting-cycle-grid">
                {campaigns.map((campaign) => (
                  ...
                  >
                    <div className="reporting-cycle-title">{campaign.title}</div>
                    <div className="reporting-cycle-meta">
                      <span>Scope: {campaign.scope}</span>
                      {campaign.due_date && <span>Due: {campaign.due_date}</span>}
                    </div>
                    <div className="reporting-cycle-footer">
                      <span className={`reporting-status-badge ${campaign.status.toLowerCase()}`}>
                        {campaign.status}
                      </span>
                      {campaign.created_at && (
...
export default AdminReportingSurveysPage;
