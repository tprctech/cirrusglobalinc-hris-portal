...
def _build_template_sections(sections_data: list[SectionIn]) -> list[ReviewTemplateSection]:
    ...
    for s in sections_data:
        section = ReviewTemplateSection(label=s.label, sort_order=s.sort_order, source_question_set_id=s.source_question_set_id)
...
def list_review_templates(db: Session = Depends(get_db)):
    return db.query(ReviewTemplate).filter(ReviewTemplate.is_deleted == False).order_by(ReviewTemplate.id).all()


@router.get("/templates/{template_id}", response_model=TemplateOut)
def get_review_template(template_id: int, db: Session = Depends(get_db)):
    row = db.query(ReviewTemplate).filter(ReviewTemplate.id == template_id, ReviewTemplate.is_deleted == False).first()
    if not row:
...
def list_review_question_sets(db: Session = Depends(get_db)):
    return db.query(ReviewQuestionSet).filter(ReviewQuestionSet.is_deleted == False).order_by(ReviewQuestionSet.id).all()
...
def get_review_question_set(qs_id: int, db: Session = Depends(get_db)):
    row = db.query(ReviewQuestionSet).filter(ReviewQuestionSet.id == qs_id, ReviewQuestionSet.is_deleted == False).first()
    if not row:
